...
object CatsTaglessSmells:
  ...
  def smell8[F[_]: Monad](fa: F[String], fb: F[Boolean], fc: F[String]): F[String] =
  ...
  def smell10[F[_]](cond: Boolean)(implicit F: MonadError[F, Throwable]): F[Unit] =
  ...
  def smell11[F[_]](cond: Boolean)(implicit F: MonadError[F, Throwable]): F[Unit] =
    if (!cond) F.raiseError(new Exception("error")) else F.unit

  // from_option: opt.fold(F.raiseError(err))(F.pure) -> F.fromOption(opt)(err) (requires MonadError)
  def smell12[F[_]](opt: Option[Int])(implicit F: MonadError[F, Throwable]): F[Int] =
  ...
  def smell14[F[_]](fa: F[Int])(implicit F: MonadError[F, Throwable]): F[String] =
    fa.attempt.map {
  ...
  def smell15[F[_]](fa: F[Int], fallback: F[Int])(implicit F: MonadError[F, Throwable]): F[Int] =
    fa.attempt.flatMap {
      case Right(a) => F.pure(a)
